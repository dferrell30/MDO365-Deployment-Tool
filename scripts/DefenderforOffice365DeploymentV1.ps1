...
function Disable-RuleEnforcement {
  param(
  ...
  switch ($RuleType) {
    'SafeLinks' {
      if (Get-Command Disable-SafeLinksRule -ErrorAction SilentlyContinue) {
        Disable-SafeLinksRule -Identity $identity -Confirm:$false -ErrorAction SilentlyContinue
      } else {
        Set-RuleEnabled -CmdletName 'Set-SafeLinksRule' -BaseParams $BaseParams -Enabled:$false
      }
    }
    'SafeAttachment' {
      if (Get-Command Disable-SafeAttachmentRule -ErrorAction SilentlyContinue) {
        Disable-SafeAttachmentRule -Identity $identity -Confirm:$false -ErrorAction SilentlyContinue
      } else {
        Set-RuleEnabled -CmdletName 'Set-SafeAttachmentRule' -BaseParams $BaseParams -Enabled:$false
      }
    }
    'AntiPhish' {
      if (Get-Command Disable-AntiPhishRule -ErrorAction SilentlyContinue) {
        Disable-AntiPhishRule -Identity $identity -Confirm:$false -ErrorAction SilentlyContinue
      } else {
        Set-RuleEnabled -CmdletName 'Set-AntiPhishRule' -BaseParams $BaseParams -Enabled:$false
      }
    }
    'HostedContentFilter' {
      if (Get-Command Disable-HostedContentFilterRule -ErrorAction SilentlyContinue) {
        Disable-HostedContentFilterRule -Identity $identity -Confirm:$false -ErrorAction SilentlyContinue
      } elseif (Supports-Param 'Set-HostedContentFilterRule' 'State') {
        Set-HostedContentFilterRule -Identity $identity -State Disabled
      } elseif (Supports-Param 'Set-HostedContentFilterRule' 'Enabled') {
        Set-HostedContentFilterRule -Identity $identity -Enabled:$false
      } else {
        Set-HostedContentFilterRule @BaseParams
      }
    }
    'HostedOutboundSpamFilter' {
      if (Get-Command Disable-HostedOutboundSpamFilterRule -ErrorAction SilentlyContinue) {
        Disable-HostedOutboundSpamFilterRule -Identity $identity -Confirm:$false -ErrorAction SilentlyContinue
      } elseif (Supports-Param 'Set-HostedOutboundSpamFilterRule' 'State') {
        Set-HostedOutboundSpamFilterRule -Identity $identity -State Disabled
      } elseif (Supports-Param 'Set-HostedOutboundSpamFilterRule' 'Enabled') {
        Set-HostedOutboundSpamFilterRule -Identity $identity -Enabled:$false
      } else {
        Set-HostedOutboundSpamFilterRule @BaseParams
      }
    }
    'MalwareFilter' {
      if (Get-Command Disable-MalwareFilterRule -ErrorAction SilentlyContinue) {
        Disable-MalwareFilterRule -Identity $identity -Confirm:$false -ErrorAction SilentlyContinue
      } else {
        Set-RuleEnabled -CmdletName 'Set-MalwareFilterRule' -BaseParams $BaseParams -Enabled:$false
      }
    }
  }
}
...
function Ensure-SafeLinksRuleGlobal {
  ...
  if (-not $rule) {
    ...
    if (Supports-Param 'New-SafeLinksRule' 'Enabled') {
      New-SafeLinksRule -Name $RuleName -SafeLinksPolicy $PolicyName -RecipientDomainIs $RecipientDomains -Enabled:$false
    } else {
  ...
  Disable-RuleEnforcement -RuleType 'SafeLinks' -BaseParams $bp
}
...
function Ensure-SafeAttachmentsRuleGlobal {
  ...
  if (-not $rule) {
    ...
    if (Supports-Param 'New-SafeAttachmentRule' 'Enabled') {
      New-SafeAttachmentRule -Name $RuleName -SafeAttachmentPolicy $PolicyName -RecipientDomainIs $RecipientDomains -Enabled:$false
    } else {
  ...
  Disable-RuleEnforcement -RuleType 'SafeAttachment' -BaseParams $bp
}
...
function Ensure-AntiPhishRuleGlobal {
  ...
  if (-not $rule) {
    ...
    if (Supports-Param 'New-AntiPhishRule' 'Enabled') {
      New-AntiPhishRule -Name $RuleName -AntiPhishPolicy $PolicyName -RecipientDomainIs $RecipientDomains -Enabled:$false
    } else {
  ...
  Disable-RuleEnforcement -RuleType 'AntiPhish' -BaseParams $bp
}
...
function Ensure-AntiSpamInboundRuleGlobal {
  ...
  if (-not $rule) {
    ...
    if (Supports-Param 'New-HostedContentFilterRule' 'Enabled') {
      New-HostedContentFilterRule -Name $RuleName -HostedContentFilterPolicy $PolicyName -RecipientDomainIs $RecipientDomains -Enabled:$false
    } else {
    ...
    Set-HostedContentFilterRule -Identity $RuleName -HostedContentFilterPolicy $PolicyName -RecipientDomainIs $RecipientDomains
  }
  $bp = @{ Identity = $RuleName; HostedContentFilterPolicy = $PolicyName; RecipientDomainIs = $RecipientDomains }
  Disable-RuleEnforcement -RuleType 'HostedContentFilter' -BaseParams $bp
}
...
function Ensure-AntiSpamOutboundRuleGlobal {
  ...
  if (-not $rule) {
    ...
    if (Supports-Param 'New-HostedOutboundSpamFilterRule' 'Enabled') {
      New-HostedOutboundSpamFilterRule -Name $RuleName -HostedOutboundSpamFilterPolicy $PolicyName -SenderDomainIs $SenderDomains -Enabled:$false
    } else {
  ...
  $bp = @{ Identity = $RuleName; HostedOutboundSpamFilterPolicy = $PolicyName; SenderDomainIs = $SenderDomains }
  Disable-RuleEnforcement -RuleType 'HostedOutboundSpamFilter' -BaseParams $bp
}
...
function Ensure-AntiMalwareRuleGlobal {
  ...
  if (-not $rule) {
    ...
    if (Supports-Param 'New-MalwareFilterRule' 'Enabled') {
      New-MalwareFilterRule -Name $RuleName -MalwareFilterPolicy $PolicyName -RecipientDomainIs $RecipientDomains -Enabled:$false
    } else {
  ...
  Disable-RuleEnforcement -RuleType 'MalwareFilter' -BaseParams $bp
}
